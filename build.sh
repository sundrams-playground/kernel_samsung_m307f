...
if [[ ! -z ${BUILD_KERNEL_BRANCH} ]]; then
	...
	if [[ ${BUILD_KERNEL_BRANCH} == "mainline" ]]; then
		...
		if [[ ${BUILD_KERNEL_MAGISK} == 'true' ]]; then
			FILE_OUTPUT=Mint-${BUILD_DEVICE_NAME}-${KERNEL_BUILD_VERSION}.A${BUILD_ANDROID_PLATFORM}_${FILE_KERNEL_CODE}${ZIP_ONEUI_VERSION}-Magisk_${BUILD_DEVICE_NAME^}.zip
        elif [[ ${BUILD_KERNEL_KERNELSU} == 'true' ]]; then
			FILE_OUTPUT=KernelSU-${BUILD_DEVICE_NAME}-v${KSU_VERSION}_${KERNEL_BUILD_VERSION}.A${BUILD_ANDROID_PLATFORM}_${FILE_KERNEL_CODE}${ZIP_ONEUI_VERSION}-${BUILD_DEVICE_NAME^}.zip
		else
			FILE_OUTPUT=Mint-${BUILD_DEVICE_NAME}-${KERNEL_BUILD_VERSION}.A${BUILD_ANDROID_PLATFORM}_${FILE_KERNEL_CODE}${ZIP_ONEUI_VERSION}-NoRoot_${BUILD_DEVICE_NAME^}.zip
		fi
	else
		...
		if [[ ${BUILD_KERNEL_MAGISK} == 'true' ]]; then
			FILE_OUTPUT=MintBeta-${BUILD_DEVICE_NAME}-${GITHUB_RUN_NUMBER}.A${BUILD_ANDROID_PLATFORM}.${FILE_KERNEL_CODE}${ZIP_ONEUI_VERSION}-${FILE_NAME_SELINUX}-Magisk_${BUILD_DEVICE_NAME^}.CI.zip
        elif [[ ${BUILD_KERNEL_KERNELSU} == 'true' ]]; then
			FILE_OUTPUT=Mint-KSU-${BUILD_DEVICE_NAME}-v${KSU_VERSION}.${GITHUB_RUN_NUMBER}-A${BUILD_ANDROID_PLATFORM}-${FILE_KERNEL_CODE}${ZIP_ONEUI_VERSION}-${FILE_NAME_SELINUX}.zip
		else
			FILE_OUTPUT=MintBeta-${BUILD_DEVICE_NAME}-${GITHUB_RUN_NUMBER}.A${BUILD_ANDROID_PLATFORM}.${FILE_KERNEL_CODE}${ZIP_ONEUI_VERSION}-${FILE_NAME_SELINUX}-NoRoot_${BUILD_DEVICE_NAME^}.CI.zip
		fi
	fi
else
	if [[ ${BUILD_KERNEL_MAGISK} == 'true' ]]; then
		FILE_OUTPUT=Mint-${BUILD_DEVICE_NAME}-${BUILD_DATE}.A${BUILD_ANDROID_PLATFORM}_${FILE_KERNEL_CODE}${ZIP_ONEUI_VERSION}_${BUILD_DEVICE_NAME^}_UB.zip
    elif [[ ${BUILD_KERNEL_KERNELSU} == 'true' ]]; then
		FILE_OUTPUT=Mint-${BUILD_DEVICE_NAME}-${BUILD_DATE}.A${BUILD_ANDROID_PLATFORM}_${FILE_KERNEL_CODE}${ZIP_ONEUI_VERSION}_${BUILD_DEVICE_NAME^}_UB.zip
	else
		FILE_OUTPUT=Mint-${BUILD_DEVICE_NAME}-${BUILD_DATE}.A${BUILD_ANDROID_PLATFORM}_${FILE_KERNEL_CODE}${ZIP_ONEUI_VERSION}_${BUILD_DEVICE_NAME^}_UB.zip
