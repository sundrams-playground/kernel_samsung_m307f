...
if [[ ! -z ${BUILD_KERNEL_BRANCH} ]]; then
	...
	if [[ ${BUILD_KERNEL_BRANCH} == "mainline" ]]; then
		...
		if [[ ${BUILD_KERNEL_MAGISK} == 'true' ]]; then
			...
			FILE_OUTPUT=KernelSU-v${KSU_VERSION}_${KERNEL_BUILD_VERSION}.A${BUILD_ANDROID_PLATFORM}_${FILE_KERNEL_CODE}${ZIP_ONEUI_VERSION}-${BUILD_DEVICE_NAME^}.zip
		else
		...
		if [[ ${BUILD_KERNEL_MAGISK} == 'true' ]]; then
			...
			FILE_OUTPUT=KernelSU-v${KSU_VERSION}_${GITHUB_RUN_NUMBER}.A${BUILD_ANDROID_PLATFORM}.${FILE_KERNEL_CODE}${ZIP_ONEUI_VERSION}-${FILE_NAME_SELINUX}-${BUILD_DEVICE_NAME^}.CI.zip
		else
